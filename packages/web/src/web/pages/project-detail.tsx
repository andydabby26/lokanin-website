...
export default function ProjectDetail() {
  ...
  if (!project) {
    return (
      <Layout>
        <div className="min-h-[70svh] flex items-center justify-center px-6 text-center font-display text-xl sm:text-2xl uppercase">
          Project not found
        </div>
      </Layout>
    );
  ...
  return (
    <Layout>
      <section className="relative min-h-[100svh] overflow-hidden">
        ...
        />
        <div className="absolute bottom-8 sm:bottom-12 left-0 right-0 z-20 flex justify-center text-center px-4 sm:px-6">
          <motion.div
          ...
          >
            ...
            <h1 className="font-display text-[clamp(2rem,10vw,6rem)] font-bold tracking-tighter uppercase text-center break-words px-2">
              {project.title}
            </h1>
          </motion.div>
        </div>
      ...
      <section className="py-14 sm:py-16 md:py-24 px-4 sm:px-6 md:px-24">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-16">
          <div className="md:col-span-8">
            <div className="max-w-4xl space-y-10 sm:space-y-12 md:space-y-16">
              {project.videoUrl && (
                <div className="aspect-video w-full bg-neutral-900 overflow-hidden relative group rounded-sm sm:rounded-none">
                    ...
                    {!isPlaying ? (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-full border border-white/20 bg-black/40 flex items-center justify-center backdrop-blur-sm group-hover:scale-110 group-hover:bg-white group-hover:border-white transition-all duration-500">
                            <Play className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-white fill-white group-hover:text-black group-hover:fill-black transition-colors" />
                          </div>
                        </div>
                      </motion.div>
                    ) : (
              ...
              {project.note && (
                <div className="pt-4 sm:pt-8 md:pt-0">
                  ...
                  <div className="font-serif text-lg sm:text-xl md:text-3xl leading-relaxed italic text-foreground/90 space-y-5 sm:space-y-6 md:space-y-8">
                    {project.note?.split('\n').map((para, i) => (
                      <p key={i}>{para}</p>
                    ))}
                  </div>
          ...
          <div className="md:col-span-4 border-t md:border-t-0 md:border-l border-white/5 pt-10 sm:pt-12 md:pt-0 md:pl-8 lg:pl-16 space-y-10 sm:space-y-12 md:space-y-16">
            <div>
      ...
      <section className="py-16 sm:py-20 md:py-32 px-4 sm:px-6 md:px-24 border-t border-white/5 flex justify-center">
        <Link href="/#works" className="group flex items-center gap-6 min-h-[48px]">
          ...
          <span className="font-sans text-[10px] tracking-[0.4em] uppercase hover:text-muted-foreground transition-colors text-center">Back to Featured Work</span>
        </Link>
      </section>
    </Layout>
  );
